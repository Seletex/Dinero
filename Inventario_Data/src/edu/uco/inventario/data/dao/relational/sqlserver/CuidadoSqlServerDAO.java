...
public class CuidadoSqlServerDAO extends DAORelational implements CuidadoDAO {

	public CuidadoSqlServerDAO(final Connection connection) {
		super(connection);
	}

	@Override
	public final void create(CuidadoDTO person) {
		final var sql = "INSERT INTO ALMACEN(  id,   descripcion, ) VALUES (?, ?)";

		try (final var preparedStatement = getConnection().prepareStatement(sql)) {

			preparedStatement.setString(1, person.getIdAsString());
			preparedStatement.setString(2, person.getDescripcion());
	...
	@Override
	public final List<CuidadoDTO> find(CuidadoDTO budget) {
	...
	private final List<CuidadoDTO> prepareAndExecuteQuery(final StringBuilder sqlBuilder,
	...
	private final void createSelectFrom(final StringBuilder sqlBuilder) {
		...
		sqlBuilder.append(" FROM Cuidado Cui");
	

	}

	private final void createWhere(final StringBuilder sqlBuilder,
			final CuidadoDTO budget, final List<Object> parameters) {
	...
	private final List<CuidadoDTO> executeQuery(PreparedStatement preparedStatement) {
	...
	private final CuidadoDTO fillCuidadoDTO(final ResultSet resultSet) {
		try {
			...
			return CuidadoDTO.create(resultSet.getString("IdNombre"),  resultSet.getString("IdDescripcion")
					);
		} catch (final DataCustomException exception) {
			throw exception;
		} catch (final SQLException exception) {
	...
	private final List<CuidadoDTO> fillResults(final ResultSet resultSet) {
		try {
			var results = new ArrayList<CuidadoDTO>();
